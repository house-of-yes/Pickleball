...
async function applyCurrentBuffer() {
  ...
  // Compute a path relative to the workspace; if none, fall back to filename
  let relPath = vscode.workspace.asRelativePath(doc.uri, false);
  if (!relPath || relPath === doc.fileName) {
    relPath = doc.fileName.split(/[\\/]/).pop() || 'untitled';
  }
...
function connectEvents(context: vscode.ExtensionContext) {
  ...
  try {
    // @ts-ignore Node >=18 in VS Code extension host has WebSocket
